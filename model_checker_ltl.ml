...
(*Fin de la mise sous FNN*)
(*-----------------------*)

  
(*IV.1. Début de la conversion LTL -> Automate de Buchi*)
...
(* Renvoie incoming_u en version modifiée de tels manière que Incoming_u(q) := Incoming_u(q) ∪ incoming *)
...
(* Renvoie true si ∃q ∈ nodeset / q.next=node.next et q.old=node.old et dans ce cas la : modifie 
...
(*V. Début de l'intersection d'automates *)
...
(*V. Fin de l'intersection d'automates *)
(*---------------------------------------*)
...
(*VI. Fin du test de vacuité*)

(*Model checker qui décide si la structure de kripke k vérifie f*)
